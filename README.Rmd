...
- want to compare optimizers that are already covered by [`{optimx}`](https://CRAN.R-project.org/package=optimx) (they provide a framework to compare about 30 optimizers),
...
- compare any optimizer function (also those not covered by `{optimx}` or other frameworks),
...
👉️ See the [package vignette](https://loelschlaeger.de/optimizeR/articles/optimizeR.html) for more details on these benefits and how we implemented them.
...
Let ![equation](https://latex.codecogs.com/svg.image?&space;f:\mathbb{R}^4\to\mathbb{R}) with
...
We aim to further improve the package.🔧 The following issues are currently on our agenda:
